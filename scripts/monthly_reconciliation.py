"""
iTech Monthly Accrual Reconciliation - PRODUCTION SUITE
Complete end-to-end automation with all advanced features
"""
...
class AdvancedMatchingEngine:
    ...
    def match_exact_id(pivot_df: pd.DataFrame, accrual_df: pd.DataFrame) -> pd.DataFrame:
        ...
        accrual_sel = accrual_sel.drop_duplicates(subset=['applicant_number'])
        
        merged = pivot_df.merge(accrual_sel, on='applicant_number', how='left', indicator=True)
        merged['match_strategy'] = merged['_merge'].apply(lambda x: 'EXACT_ID' if x == 'both' else None)
        merged['match_confidence'] = merged['match_strategy'].apply(lambda x: 100 if x == 'EXACT_ID' else 0)
        return merged
    
    @staticmethod
    def match_hybrid(merged_df: pd.DataFrame, accrual_df: pd.DataFrame) -> pd.DataFrame:
        """Strategy 2: Name + ID hybrid"""
        unmatched_idx = merged_df[merged_df['accrual_name'].isna()].index
    ...
    def match_fuzzy(merged_df: pd.DataFrame, accrual_df: pd.DataFrame) -> pd.DataFrame:
        """Strategy 3: Fuzzy name fallback (85% threshold)"""
        unmatched_idx = merged_df[merged_df['accrual_name'].isna()].index
...
class VarianceCalculator:
    ...
    def calculate_all_variances(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Match status - CRITICAL FIX: Use accrual_name field
        df['Status'] = df.apply(lambda row: 
            '✅ Matched' if pd.notna(row.get('accrual_name')) and row.get('accrual_name') != '' else
        ...
        def get_reason(row):
            if row['Status'] == '✅ Matched':
                ...
                return f"Not in accrual (Vendor: {row.get('vendor_name', '?')})"
            elif row['Status'] == '❌ Missing in Pivot':
                return 'Accrual without transaction'
            else:
...
class AccrualSuggestionGenerator:
    ...
    def generate_suggestions(df: pd.DataFrame) -> pd.DataFrame:
        ...
        suggestions = pd.DataFrame({
            ...
            'vendor_name': unmatched.get('vendor_name', ''),
            'service_period': unmatched.get('invoice_date', ''),
...
class MonthlyOrchestrator:
    ...
    def run(self) -> Dict:
        ...
        try:
            ...
            self._print_summary(metrics, len(suggestions), match_rate)
    ...
    def _generate_reports(self, matched, suggestions) -> Tuple[Path, Path]:
        ...
        with pd.ExcelWriter(report_file, engine='openpyxl') as writer:
            # Executive Summary
            summary_data = {
                'Metric': ['Total Records', 'Matched', 'Unmatched', 'Match Rate (%)', 'Total Billed', 'Total Paid', 'Variance'],
                'Value': [
                    len(matched),
            ...
            # Variance Analysis
            variance_cols = ['consultant_name', 'vendor_name', 'hours_variance', 'amount_variance', 'variance_severity', 'Status']
            variance_data = matched[[c for c in variance_cols if c in matched.columns]].copy()
    ...
    def _print_summary(self, metrics, suggestions_count, match_rate):
        """Print final summary"""
        self.logger.info("\n" + "="*80)
        ...
        self.logger.info(f"   Matched: {metrics['matched_count']:,} ({match_rate:.1f}%)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python monthly_reconciliation.py --month may --year 2025
  python monthly_reconciliation.py --month june --year 2025
        """
    )
